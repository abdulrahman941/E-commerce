...
import { schema } from '@/schema/VerifyResetCode'
...
async function handleVerifyResetCode(values: FormData){
   setisLoading(true)
 try {
   let {data} =await axios.post(`https://ecommerce.routemisr.com/api/v1/auth/verifyResetCode`,values)
  console.log(data);
  if(data.status=='Success'){
    toast.success("ResetCode is successfully")
  ...
  <div className="container mx-auto my-12 sm:[35%] md:[50] lg:w-[60%] bg-gray-100 rounded-md py-5">
    <h1 className='text-green-600 text-3xl text-center'>Verify Reset Code</h1>
      ...
      <FieldGroup>
        <Field>
          <FieldLabel htmlFor="email">Reset-Code:</FieldLabel>
          <Input {...form.register('resetCode')} id="Code" type="string" placeholder="enter the Reset-Code" />
          {form.formState.errors.resetCode && <p className="text-red-500">{form.formState.errors.resetCode.message}</p>}
        </Field>
      </FieldGroup>
    </FieldSet>
      </div>
